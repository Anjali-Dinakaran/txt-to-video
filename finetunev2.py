...
class EnhancedTextToVideoTrainer:
    ...
    def setup_models(self):
        ...
        try:
            ...
            # Try loading the full pipeline first, then extract UNet
            try:
                self.logger.info("Attempting to load full SVD pipeline...")
                full_pipeline = StableVideoDiffusionPipeline.from_pretrained(
                    "stabilityai/stable-video-diffusion-img2vid-xt",
                    torch_dtype=torch.float16 if self.device.type == 'cuda' else torch.float32,
                    variant="fp16" if self.device.type == 'cuda' else None
                )
                self.unet = full_pipeline.unet
                self.scheduler = full_pipeline.scheduler
                ...
                torch.cuda.empty_cache() if torch.cuda.is_available() else None
                    ...
                    self.logger.warning(f"Pipeline loading failed: {pipeline_error}")
                    self.logger.info("Attempting alternative UNet loading...")
                    
                    # Alternative: Create UNet with manual config
                    try:
                        # Create UNet with compatible configuration
                        unet_config = {
                            "sample_size": 64,  # Will be scaled
                            "in_channels": 8,
                            "out_channels": 4,
                            "down_block_types": [
                            ...
                            "num_attention_heads": None,  # Avoid the naming conflict
                        ...
                        self.logger.warning(f"Manual config failed: {config_error}")
                        self.logger.info("Creating minimal UNet configuration...")
                        ...
                        self.unet = UNet3DConditionModel(**minimal_config)
            ...
            try:
                ...
                elif hasattr(self.unet, 'set_attn_processor'):
                    from diffusers.models.attention_processor import AttnProcessor2_0
                    self.unet.set_attn_processor(AttnProcessor2_0())
                    self.logger.info("Attention 2.0 processor enabled")
            except Exception as attn_error:
                self.logger.warning(f"Could not enable memory efficient attention: {attn_error}")
            
            # Log model info
            total_params = sum(p.numel() for p in self.unet.parameters())
            trainable_params = sum(p.numel() for p in self.unet.parameters() if p.requires_grad)
            self.logger.info(f"UNet loaded - Total params: {total_params:,}, Trainable: {trainable_params:,}")
            
            self.logger.info("Models loaded successfully")
            
        except Exception as e:
            self.logger.error(f"Error loading models: {e}")
            self.logger.error("Consider updating diffusers: pip install --upgrade diffusers")
            raise
